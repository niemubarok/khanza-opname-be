...
import StokBeforeOpname from "App/Models/StokBeforeOpname";
...
export default class OpnameController {
  ...
  public async barang({ response }: HttpContextContract) {
  ...
  public async store({ request, response }: HttpContextContract) {
    ...
    //ambil stok dari stok_before_opname
    const stokAwal = await Database.from("stok_before_opname")
      .where({
        kode_brng,
        kd_bangsal,
        tanggal,
    ...
    const defineStok = () => {
      ...
      if (isOpnamed[0]?.real === null) {
        ...
        return stokFromGudangBarang[0]?.stok
          ? stokFromGudangBarang[0]?.stok
          : 0;
      } else {
        //jika sudah opname ambil stok dari stok_before_opname
        return stokAwal[0]?.stok ? stokAwal[0]?.stok : 0;
      }
    };
    ...
    const real =
      isOpnamed[0]?.real === null ? req.real : isOpnamed[0]?.real + req.real;
    ...
    const keterangan =
      isOpnamed[0]?.real === null
        ? "Via web oleh " + req.petugas
        : "Obat tambahan oleh " + req.petugas;

    const no_batch = "";
    const no_faktur = "";
    const petugas = req.petugas;
    const stok_awal = stokFromGudangBarang[0]?.stok
      ? stokFromGudangBarang[0]?.stok
      : 0;
    ...
    try {
      ...
      const tempOpname = await StokBeforeOpname.query()
        .where({
          kode_brng,
          kd_bangsal,
          tanggal,
      ...
      if (tempOpname === null) {
        await Database.table("stok_before_opname").insert({
          kode_brng,
          kd_bangsal,
          tanggal,
          stok,
      ...
      try {
        ...
        try {
          ...
          if (expire) {
            try {
              await Database.from("databarang")
                .where({
                  kode_brng,
                })
                .update({
          ...
          console.log("gagal menyimpan ke tabel riwayat");
          console.log(error);
